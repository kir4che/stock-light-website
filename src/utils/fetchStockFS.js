...
const fetchCashFlowStatement = async ({ stockId, setIsLoading }) => {
	setIsLoading(true)

	try {
		const response = await fetch(`${process.env.DB_URL}/api/stock/financial/cashFlowStatement/${stockId}`, {
			method: 'GET',
		})
		const data = await response.json()
...
export { fetchAssetStatement, fetchCashFlowStatement, fetchIncomeStatement, fetchLiabilitiesEquity }
