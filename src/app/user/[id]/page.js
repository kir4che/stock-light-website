					...
					{session.provider !== 'facebook' && session.provider !== 'google' && (
						<>
							...
							<div className='flex text-center'>
								<button
									type='button'
									onClick={() => router.push(`/user/${session.user.id}/portfolio`)}
								...
								<button
									type='button'
									onClick={() => router.push(`/user/${session.user.id}/lightHistory`)}
									className='w-1/3 p-3 focus:outline-none hover:bg-zinc-900/10 hover:dark:bg-zinc-900/60'
								>
									查詢點燈紀錄
								</button>
								...
								<button
									type='button'
									onClick={() => router.push(`/user/${session.user.id}/changePassword`)}
									className='w-1/3 p-3 focus:outline-none hover:bg-zinc-900/10 hover:dark:bg-zinc-900/60'
								>
									修改密碼
								</button>
							...
							<div className='text-center'>
								<button
									type='button'
									onClick={() => router.push(`/user/${session.user.id}/cardHistory`)}
